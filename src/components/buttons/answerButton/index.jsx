...
function AnswerButton({ answer, handleClick, selectedAnswer, className, transitionDelay, onDragStart }) {
    ...
    const handleTouchMove = (e) => {
        ...
        if (touchDragging) {
            ...
            if (clone) {
                clone.style.left = `${touch.clientX - (answerRef.current.offsetWidth / 2)}px`;
    ...
    return (
        <div>
            <motion.div
                ...
                className={`${selectedAnswer === answer ? className : "_tr_answer"} ${isDragging || touchDragging ? "_tr_answer_dragging" : ""}`}
                draggable="true"
